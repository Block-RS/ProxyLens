...
class Explorer(object):
    ...
    def __init__(self, host='localhost', port=8545, tls=False, max_retries=3, full_url=None):
        self.scheme = 'https' if tls else 'http'
        self.host = host
        self.port = port
        self.session = requests.Session()
        self.session.mount(f"{self.scheme}://", HTTPAdapter(max_retries=max_retries))
        self.headers = {'Content-Type': 'application/json'}

        # ✅ 若提供 full_url，直接使用，不拼接端口
        if full_url:
            self.url = full_url
        else:
            self.url = f"{self.scheme}://{self.host}:{self.port}"

        # self.url = "https://mainnet.infura.io/v3/ebec0b91a8a943ab8de8576000e5b9a9"

        print(f"Explorer URL: {self.url}")
